...
Clazz.load(["javajs.api.GenericCifDataParser", "java.util.Hashtable", "JU.SB"], "JU.CifDataParser", ["JU.Lst"], function(){
...
this.nTypesRemaining = 2147483647;
Clazz.instantialize(this, arguments);}, JU, "CifDataParser", null, javajs.api.GenericCifDataParser);
...
function(nullString){
JU.CifDataParser.nullString = nullString;
}, "~S");
...
function(){
return (this.fileHeader == null ? "" : this.fileHeader.toString());
});
Clazz.overrideMethod(c$, "getAllCifData", 
function(){
return this.getAllCifDataType([]);
});
Clazz.overrideMethod(c$, "getAllCifDataType", 
function(types){
if (types != null) {
if (types.length == 0) {
types = null;
...
types[i] = this.fixKey(types[i]);
}
}}this.nTypesRemaining = (types == null ? 2147483647 : types.length);
this.line = "";
var key;
var data = null;
var data0 = null;
...
this.getAllCifLoopData(data, types);
if (this.nTypesRemaining == 0) break;
continue;
...
key = this.fixKey(key);
if (types == null || this.checkKey(types, key)) data.put(key, value);
}}}
} catch (e) {
...
if (key.equals(types[i])) this.nTypesRemaining--;
return true;
}}
return false;
}, "~A,~S");
...
if ((str.charAt(0)).charCodeAt(0) != 0) ret.append(str).append(" ");
if ((++m % n) == 0) ret.append("\n");
}
...
if (this.wasUnquoted && (typeof(o)=='string')) {
var str = o;
...
var c = (data.charAt(pt + 1)).charCodeAt(0);
...
this.columnCount = 0;
break;
}if (!((typeof(o)=='string')) || (o).charAt(0) != '_') break;
pt = this.columnCount++;
...
if ((o = this.peekToken()) == null || !((typeof(o)=='string')) || !(o).startsWith(str0)) break;
key = null;
}
...
function(key){
return (key.charAt(1) == 'm' && key.startsWith("_magnetic") ? key.substring(9) : key.charAt(1) == 'j' && key.startsWith("_jana") ? key.substring(5) : key).$replace('.', '_').toLowerCase();
}, "~S");
...
if (this.ich == ichStart + 1) if (JU.CifDataParser.nullString != null && (this.str.charAt(ichStart) == '.' || this.str.charAt(ichStart) == '?')) return JU.CifDataParser.nullString;
...
var o;
var havedata = (types == null);
while ((o = this.peekToken()) != null && (typeof(o)=='string') && (o).charAt(0) == '_') {
key = this.fixKey(this.getTokenPeeked());
keyWords.addLast(key);
if (types == null || this.checkKey(types, key)) {
data.put(key,  new JU.Lst());
havedata = true;
}}
var skipping = !havedata;
...
var cdata = data.get(keyWords.get(i));
if (cdata != null) cdata.addLast(this.columnData[i]);
}
}
}this.isLoop = false;
}, "java.util.Map,~A");
c$.nullString = "\0";
c$.htFields =  new java.util.Hashtable();
});
