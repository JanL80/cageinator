...
c$.toFloatArray = Clazz_defineMethod(c$, "toFloatArray", 
function(ds){
var f =  Clazz_newFloatArray (ds.length, 0);
for (var i = ds.length; --i >= 0; ) f[i] = ds[i];
...
;//5.0.1-v7 Wed Jun 03 09:22:38 CDT 2026
Clazz_declarePackage("J.adapter.readers.pymol");
Clazz_load(null, "J.adapter.readers.pymol.JmolObject", ["java.util.Hashtable", "JU.P3", "$.PT", "$.SB", "J.adapter.readers.pymol.PyMOL", "$.PyMOLReader", "JU.BSUtil", "$.Escape"], function(){
var c$ = Clazz_decorateAsClass(function(){
this.shape = 0;
this.bsAtoms = null;
this.info = null;
...
Clazz_defineMethod(c$, "getDoubleOrDefault", 
function(key){
return this.doubleSetting(key);
}, "~N");
Clazz_defineMethod(c$, "getIntOrDefault", 
function(key){
return Clazz_doubleToInt(this.getDoubleOrDefault(key));
}, "~N");
Clazz_defineMethod(c$, "getBooleanOrDefault", 
function(key){
...
Clazz_defineMethod(c$, "addJmolObjectNoInfo", 
...
var icolor = Clazz_doubleToInt(this.getUniqueDoubleDef(uniqueID, 66));
...
Clazz_defineMethod(c$, "getUniqueDoubleDef", 
function(id, key){
...
Clazz_defineMethod(c$, "doubleSetting", 
function(i){
var setting = this.getSetting(i);
if (setting != null && setting.size() == 3) return (setting.get(2)).doubleValue();
return J.adapter.readers.pymol.PyMOL.getDefaultSetting(i, this.pymolVersion);
...
if (offset == null) offset = J.adapter.readers.pymol.PyMOLScene.setLabelPosition(this.labelPosition,  Clazz_newFloatArray (7, 0), false);
...
this.addVersionSetting(471, 4,  Clazz_newDoubleArray(-1, [1, 1, 0]));
...
jo = this.addJmolObject(135180, bs,  Clazz_newArray(-1, [this.getBooleanOrDefault(156) ? "FULLYLIT" : "FRONTLIT", (surfaceMode == 3 || surfaceMode == 4 ? " only" : ""), this.bsCarve, Double.$valueOf(withinDistance)]));
...
info =  Clazz_newArray(-1, [bsH, Double.$valueOf(this.getUniqueDoubleDef(repType, 605))]);
...
function(bs){
var info =  Clazz_newFloatArray(-1, [this.doubleSetting(378), this.doubleSetting(377), this.doubleSetting(382), this.doubleSetting(379), this.doubleSetting(380), this.doubleSetting(381), this.doubleSetting(581)]);
...
var scalex50 = Clazz_doubleToInt(v == 1 ? this.getUniqueDoubleDef(id, 512) * 50 : 0) & 0x3F;
...
var scalex50 = Clazz_doubleToInt(isSpecial ? this.getUniqueDoubleDef(uid, 512) * 50 : 0) & 0x3F;
