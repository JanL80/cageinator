...
Clazz.load(null, "J.adapter.readers.cif.BCIFDecoder", ["JU.BC", "$.PT", "$.SB"], function(){
...
case 70:
this.factor = J.adapter.readers.cif.BCIFDecoder.geMapInt(encoding.get("factor"), null);
this.dataType = 2;
this.mode |= 8;
break;
case 68:
this.origin = J.adapter.readers.cif.BCIFDecoder.geMapInt(encoding.get("origin"), this.key + " origin");
this.mode |= 1;
break;
case 82:
this.mode |= 2;
this.srcSize = J.adapter.readers.cif.BCIFDecoder.geMapInt(encoding.get("srcSize"), null);
break;
case 73:
this.mode |= 4;
this.packingSize = J.adapter.readers.cif.BCIFDecoder.geMapInt(encoding.get("srcSize"), null);
...
case 66:
if (this.btype != 0) {
System.err.println("!BCIFDecoder skipping " + this.key + " duplicate bytearray encoding " + this.btype + " not " + encoding);
continue;
}this.btype = J.adapter.readers.cif.BCIFDecoder.geMapInt(encoding.get("type"), null);
...
if (this.srcType == 0) {
this.srcType = J.adapter.readers.cif.BCIFDecoder.geMapInt(encoding.get("srcType"), null);
}}
}, "~A,JU.SB");
...
if ((this.mode & 2) == 2) {
run = J.adapter.readers.cif.BCIFDecoder.getTemp(this.srcSize << 1);
len = this.srcSize;
}if ((this.mode & 4) == 4) {
