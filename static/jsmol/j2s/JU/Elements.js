...
var c$ = Clazz.declareType(JU, "Elements", null);
...
c$.getCommonNMRIsotope = Clazz.defineMethod(c$, "getCommonNMRIsotope", 
function(sym){
if (JU.PT.isDigit(sym.charAt(0))) return sym;
if (sym.equals("D")) return "2H";
var pt = JU.Elements.commonNmrIsotopes.indexOf(sym + ",");
return (pt < 0 ? sym : JU.Elements.commonNmrIsotopes.substring(pt - 2, pt + sym.length).trim());
}, "~S");
c$.getNmrNucleusFromName = Clazz.defineMethod(c$, "getNmrNucleusFromName", 
function(name){
return JU.Elements.getCommonNMRIsotope(JU.Elements.elementSymbolFromNumber(JU.Elements.elementNumberFromName(name)));
}, "~S");
...
}c$.hydrophobicities =  Clazz.newFloatArray(-1, [0, 0.62, -2.53, -0.78, -0.9, 0.29, -0.85, -0.74, 0.48, -0.4, 1.38, 1.06, -1.5, 0.64, 1.19, 0.12, -0.18, -0.05, 0.81, 0.26, 1.08]);
{
