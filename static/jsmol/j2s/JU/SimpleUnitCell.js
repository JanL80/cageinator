...
c$.parseCalc = Clazz.defineMethod(c$, "parseCalc", 
function(vwr, s){
return JU.SimpleUnitCell.parseCalcFunctions(vwr, JU.SimpleUnitCell.functions, s);
}, "JV.Viewer,~S");
c$.parseCalcFunctions = Clazz.defineMethod(c$, "parseCalcFunctions", 
function(vwr, functions, s){
var parts;
if (s.startsWith("+")) s = s.substring(1);
var d = JU.PT.parseFloatStrict(s);
if (!Float.isNaN(d)) return d;
s = s.toLowerCase();
if (functions != null && s.indexOf('(') >= 0) {
parts = JU.PT.split(s, "(");
for (var i = parts.length - 1; --i >= 0; ) {
var p = parts[i];
if (p.length == 0) continue;
var f = null;
for (var j = functions.length; --j >= 0; ) {
...
if (haveDigit && !haveDecimal) parts[i] += ".0";
}
s = JU.PT.join(parts, '/', 0);
...
var pt = n;
for (var i = n; --i >= 0; ) {
switch ((part.charAt(i)).charCodeAt(0)) {
case 110:
case 111:
i--;
default:
