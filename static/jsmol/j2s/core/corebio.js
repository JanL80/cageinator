...
if (this.line.startsWith("REMARK 998")) return this.remark998();
...
Clazz_defineMethod(c$, "remark998", 
function(){
if (this.asc.isConSurf == null) {
if (this.line.indexOf("ConSurf") >= 0) this.asc.isConSurf = Boolean.TRUE;
}return true;
});
...
Clazz_overrideMethod(c$, "getBioStructureType", 
function(){
return J.c.STR.NONE;
});
...
Clazz_overrideMethod(c$, "getBioStructureSubType", 
function(dssx){
switch (dssx) {
case 1111490651:
if (this.proteinStructure != null) return this.proteinStructure.subtype;
break;
}
return J.c.STR.NONE;
}, "~N");
...
var type = monomers[++index].getBioStructureType();
var mLast = -1;
...
;//5.0.1-v7 Wed Jun 03 09:22:38 CDT 2026
Clazz_declarePackage("JM");
Clazz_load(null, "JM.BioModelSet", ["java.util.Hashtable", "JU.AU", "$.BS", "$.Lst", "$.PT", "$.SB", "J.api.Interface", "J.c.STR", "JM.Group", "$.ModelSet", "JM.BioResolver", "JS.T", "JU.BSUtil", "$.Escape", "$.Logger"], function(){
var c$ = Clazz_decorateAsClass(function(){
this.haveDSSR = false;
this.vwr = null;
this.ms = null;
...
return  new JU.BS();
case 1073741925:
return this.getAnnotationBits("domains", 1073741925, specInfo, null);
case 1073742189:
return this.getAnnotationBits("validation", 1073742189, specInfo, null);
case 1073742128:
return this.getAnnotationBits("rna3d", 1073742128, specInfo, null);
case 1073741863:
var s = specInfo;
bs =  new JU.BS();
return (s.length % 2 != 0 ? bs : this.ms.getAtomBitsMDa(1086324742, this.getAllBasePairBits(s), bs));
case 1111490587:
return this.getAnnotationBits("dssr", 1111490587, specInfo, null);
case 1086324744:
...
function(key, tok, specInfo, type){
var bs =  new JU.BS();
if (tok == 1111490587 && !this.haveDSSR) return bs;
var pa = this.vwr.getAnnotationParser(tok);
var ann;
var keyV = key + " V ";
for (var i = this.ms.mc; --i >= 0; ) {
if ((ann = this.ms.getInfo(i, key)) != null) {
var b = pa.getAtomBits(this.vwr, specInfo, (this.ms.am[i]).getCachedAnnotationMap(keyV, ann), this.ms.am[i].annotationCache, tok, i, this.ms.am[i].bsAtoms);
bs.or(b);
}}
if (tok == 1111490587 && type != null) {
var groups = this.ms.getGroupsForAtoms(bs);
for (var i = groups.size(); --i >= 0; ) {
...
Clazz_defineMethod(c$, "getStructureDSSRAssignments", 
function(){
if (!this.haveDSSR) return;
this.getAnnotationBits("dssr", 1111490587, "iloops", J.c.STR.DSSRINTERNALLOOP);
this.getAnnotationBits("dssr", 1111490587, "hairpins", J.c.STR.DSSRHAIRPINLOOP);
this.getAnnotationBits("dssr", 1111490587, "bulges", J.c.STR.DSSRBULGELOOP);
this.getAnnotationBits("dssr", 1111490587, "junctions", J.c.STR.DSSRJUNCTIONLOOP);
this.getAnnotationBits("dssr", 1111490587, "stems", J.c.STR.DSSRSTEM);
});
Clazz_defineMethod(c$, "setHaveDSSR", 
function(b){
this.haveDSSR = b;
}, "~B");
Clazz_defineMethod(c$, "getDSSRProperties", 
function(dataType){
if (!this.haveDSSR || dataType == null) return null;
var dataPath = (dataType.indexOf('.') < 0 ? "dssr" + "." + dataType : dataType);
if (this.dssrDataMap == null) {
this.dssrDataMap =  new java.util.Hashtable();
}var dataList = this.dssrDataMap.get(dataType);
if (dataList == null) {
this.dssrDataMap.put(dataType, dataList =  new JM.ModelSet.DataList(dataPath));
}for (var i = this.ms.mc; --i >= 0; ) if (this.ms.am[i].isBioModel && (this.ms.am[i]).haveDSSR) this.vwr.getAnnotationParser(1111490587).getAtomicDSSRData(this.ms, i, dataList);

return dataList;
}, "~S");
Clazz_defineMethod(c$, "getAtomicDSSRDataFloat", 
function(dataType, atomIndex){
var dl = (this.dssrDataMap == null ? null : this.dssrDataMap.get(dataType));
return (dl != null && dl.floatData != null && dl.floatData.length > atomIndex ? dl.floatData[atomIndex] : NaN);
}, "~S,~N");
Clazz_defineMethod(c$, "getAtomicDSSRDataString", 
function(dataType, atomIndex){
var dl = (this.dssrDataMap == null ? null : this.dssrDataMap.get(dataType));
return (dl != null && dl.stringData != null && dl.stringData.length > atomIndex ? dl.stringData[atomIndex] : null);
}, "~S,~N");
Clazz_defineMethod(c$, "releaseModelSet", 
function(){
System.err.println("can't remove ms here??");
});
Clazz_defineMethod(c$, "clearDSSR", 
function(){
this.dssrDataMap = null;
});
});
...
if (haveNucl && this.haveDSSR && vHBonds != null) s += this.vwr.getAnnotationParser(1111490587).getHBonds(this.ms, this.modelIndex, vHBonds, doReport);
return s;
}, "JU.Lst,~B,~B,~B,~N");
Clazz_defineMethod(c$, "calculateStructures", 
...
function(key, ann){
var cache = (this.annotationCache == null && ann != null ? this.annotationCache =  new java.util.Hashtable() : this.annotationCache);
...
this.fixAnnotations(i, 1073741925);
this.fixAnnotations(i, 1073742189);
this.fixAnnotations(i, 1111490587);
}
}, "JU.BS");
Clazz_defineMethod(c$, "fixAnnotations", 
function(modelIndex, type){
var parser = this.vwr.getAnnotationParserIfPresent(type);
if (parser == null) return;
var key = null;
switch (type) {
case 1111490587:
this.vwr.ms.bioModelset.clearDSSR();
return;
case 1073742189:
key = "validation";
break;
...
var o = this.ml.ms.getInfo(modelIndex, key);
if (o != null) {
var dbObj = (this.ms.am[modelIndex]).getCachedAnnotationMap(key, o);
if (dbObj != null) parser.fixAtoms(modelIndex, dbObj, this.bsAddedMask, type, 20);
}}, "~N,~N");
...
;//5.0.1-v7 Wed Jun 03 09:22:38 CDT 2026
Clazz_declarePackage("JM");
Clazz_load(["JM.Monomer", "J.c.STR"], "JM.PhosphorusMonomer", ["JU.Quat", "$.V3"], function(){
var c$ = Clazz_decorateAsClass(function(){
this.dssrSubType = null;
this.basepairSubType = null;
Clazz_instantialize(this, arguments);}, JM, "PhosphorusMonomer", JM.Monomer);
Clazz_prepareFields (c$, function(){
this.dssrSubType = J.c.STR.NONE;
this.basepairSubType = J.c.STR.NONE;
});
Clazz_overrideConstructor(c$, 
function(){
});
...
switch (dssx) {
case 1111490651:
return this.dssrSubType;
case 1073741863:
return this.basepairSubType;
}
return J.c.STR.NONE;
}, "~N");
Clazz_overrideMethod(c$, "setDSSRStructureSubType", 
function(type){
this.dssrSubType = type;
}, "J.c.STR");
...
function(){
if (this.bioPolymer != null && !(this.bioPolymer).isDssrSet) this.bioPolymer.model.ms.vwr.getAnnotationParser(1111490587).getBasePairs(this.bioPolymer.model.ms.vwr, this.bioPolymer.model.modelIndex);
return this.bps;
});
...
if (this.dssrNT != null) return this.dssrFrame = vwr.getAnnotationParser(1111490587).getDSSRFrame(this.dssrNT);
...
switch (this.monomers[groupIndex].getBioStructureType()) {
case J.c.STR.SHEET:
case J.c.STR.HELIX:
...
for (var i = this.monomerCount; --i >= 0; ) if ((types[i] = this.monomers[i].getBioStructureType()) === J.c.STR.TURN) types[i] = J.c.STR.NONE;
