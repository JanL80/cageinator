...
this.dpi = 300;
if (this.quality <= 0) this.quality = (params.containsKey("qualityJPG") ? (params.get("qualityJPG")).intValue() : -1);
if (this.quality >= 90) {
...
this.writeDensities();
this.writeThumbNailSize();
var comment = null;
if (jpegObj.comment != null && jpegObj.comment.length > 0) this.writeString(jpegObj.comment, 0xE1);
...
Clazz.defineMethod(c$, "writeDensities", 
function(){
this.out.writeByteAsInt(1);
this.out.writeShort(this.dpi);
this.out.writeShort(this.dpi);
});
Clazz.defineMethod(c$, "writeThumbNailSize", 
function(){
this.writeArray( Clazz.newByteArray (2, 0));
});
...
c$.jfif =  Clazz.newByteArray(-1, [0xff, 0xe0, 0, 16, 0x4a, 0x46, 0x49, 0x46, 0, 1, 0]);
